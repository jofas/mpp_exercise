program pi_recv_any
  ...
  real, dimension(:), allocatable :: pi_buff
  ...
  do j = 1, RUN_N_TIMES
    ...
    if (rank > 0) then
      ...
      do i = 1, world_size - 1
        call mpi_recv(pi_buff(i), 1, mpi_real, &
          mpi_any_source, 0, mpi_comm_world, stat, ierror)
      end do
      pi_approx = pi_approx + sum(pi_buff(:))
    end if
  end do
