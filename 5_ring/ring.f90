program ring
  ...
  type(MPI_Status)  :: stat_send, stat_recv
  ...
  do i = 1, w_size - 1
    call mpi_issend( &
      recv_, 1, mpi_integer, send_to, 0, comm, req_send &
    )
    call mpi_recv( &
      recv_, 1, mpi_integer, recv_from, 0, comm, stat_recv &
    )
    sum_ = sum_ + recv_
    call mpi_wait(req_send, stat_send)
  end do
  end_t = mpi_wtime()
